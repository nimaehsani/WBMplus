	...
	if (_MDOutDisch_t_1_ID == MFUnset) {
		...
		*/
		...
		if (resStorage + resStorageChg < resCapacity && resStorage + resStorageChg > minresStorage) {
                        SIMOUT=ANN;
                        if (SIMOUT<0){
                            printf("Error: Negative Discharge!\n");
                        }
		...
		resRelease = SIMOUT;
		//
		if (res_release_t_2 < release_min){  // Chekking MAx-Min Release  From Reservoir
			release_min = discharge;
		}
		else {
			if (res_release_t_2 > release_max){
				release_max = discharge;
			}
			else {
				release_max = discharge;
				release_min = discharge/5;
			}
		}
		res_release_t_1 = (avmtdRelease - release_min)/(release_max - release_min); 
...
int MDReservoirDef () {
	...
	case MDneuralnet:
		if (((_MDInDischargeID      = MDDischLevel2Def ()) == CMfailed) ||
			...
			// ((_MDInDischMinID    = MFVarGetID (MDVarDischargeMin,      "m3/s",  MFInput,  MFState, MFBoundary)) == CMfailed) || // Min Discharge Should be calculated and Stored During Programm Run
			// ((_MDInDischMaxID    = MFVarGetID (MDVarDischargeMax,      "m3/s",  MFInput,  MFState, MFBoundary)) == CMfailed) || // Min Discharge Should be calculated and Stored During Programm Run
